...
if __name__ == "__main__":
    ...
    q = torch.randn(1, 32, 128*1024, 128, device="cuda", dtype=torch.bfloat16)
    k = torch.randn(1, 32, 128*1024, 128, device="cuda", dtype=torch.bfloat16)
    v = torch.randn(1, 32, 128*1024, 128, device="cuda", dtype=torch.bfloat16)
    import time
    sparse1 = 1024 * 2
    for i in range(10):
        start_time = time.time()
        O1 = block_sparse_attention(q,k,v,sparse1,64,64)
        torch.cuda.synchronize()
        end_time = time.time()
        print(f'Block Sparse1 Attention: {end_time - start_time:.6f} seconds')
    ...
    for i in range(10):
        start_time = time.time()
        O2 = block_sparse_attention(q, k, v, sparse2, 64, 64)
        torch.cuda.synchronize()
        end_time = time.time()
        print(f'Block Sparse2 Attention:: {end_time - start_time:.6f} seconds')
    ...
    for i in range(10):
        ...
        torch.cuda.synchronize()
        end_time = time.time()
        print(f'cost flash_attn Attention:: {end_time - start_time:.6f} seconds')
        
    print('testing flash_decoding')
