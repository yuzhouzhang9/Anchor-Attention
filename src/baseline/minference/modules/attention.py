...
from ..ops.streaming import streaming_forward, streaming_forward2
from .flex import flex_forward
from .kv import *
...
try:
    ...
    from ..ops.flash import _flash_decoding as flash_attn_func
        ...
        **kwargs,
        ...
        if q_len != 1:
            ...
            for head in range(query_states.size(1)):
                ...
                if True or (self.is_search and self.layer_idx >= len(config_list)):
                    config[head] = search_pattern(q, k, head)
                torch.cuda.synchronize()
                end_time = time.time()
                print(f"search partten cost:{end_time - start_time}")
                ...
                if self.layer_idx >= self.starting_layer and not self.is_search:
                    ...
                    torch.cuda.synchronize()
                    end_time = time.time()
                    print(f"partten computer cost:{end_time - start_time}")
...
def gather_last_q_vertical_slash_topk_vllm(self, q, k, v, head_id):
    ...
    if self.patch_config.get("flex", False):
        return flex_forward(q, k, v, {"attn_forward_config": self.patch_config["flex_kwargs"]})
