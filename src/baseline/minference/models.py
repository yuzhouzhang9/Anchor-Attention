...
class MInference:
    ...
    def patch_model(self, model):
        if self.config.kv_type == "retrieval":
        ...
        if self.config.attn_type == "minference":
        ...
        elif self.config.attn_type in ["flex", "dense"]:
            model = new_patch(model, self.config)
            ...
            pass
        elif self.config.attn_type == "inf":
            model = patch_hf(
                model,
                attn_type="inf",
                attn_kwargs={
        ...
        elif self.config.attn_type == "vllm_flex":
            patch_config = {
                "flex": True,
                "flex_kwargs": {},
                **self.config.attn_kwargs,
