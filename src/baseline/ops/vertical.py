...
from baseline.ops.flex import triton_block_wise_attention
...
def triton_vertical_slash_attention(
    ...
    # print(f"vertical_num_blocks:{vertical_num_blocks.shape}")
    # print(f"slash_index:{slash_index.shape}")
...
if __name__ == "__main__":
    ...
    q = torch.randn(B, N, H, D, device="cuda:1", dtype=torch.bfloat16) / 0.5
    k = torch.randn(B, N, H // 4, D, device="cuda:1", dtype=torch.bfloat16) / 0.5
    v = torch.randn(B, N, H // 4, D, device="cuda:1", dtype=torch.bfloat16)
