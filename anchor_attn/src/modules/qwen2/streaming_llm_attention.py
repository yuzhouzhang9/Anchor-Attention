...
from anchor_attn.src.ops.streaming_llm_attention import streaming_llm_attention
...
def qwen2_streaming_llm_attention_forward(
    ...
    if query_states.shape[1] > 1:
        ...
        from my_utils import Logger
        Logger.log(f"self.layer_idx: {self.layer_idx}")
        Logger.log(f"query_states shape: {query_states.shape}")
        Logger.log(f"global_window: {self.config.global_window}")
        Logger.log(f"local_window: {self.config.local_window}")
