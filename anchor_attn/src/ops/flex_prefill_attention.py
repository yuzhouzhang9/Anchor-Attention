...
def triton_block_wise_prefill_attention(
    ...
    Logger.log(f"block_idx:{block_idx.shape}")
    assert q_len == k_len
...
def get_active_blocks(
    ...
    # from my_utils import # Logger
    # Logger.log(f"qk shape:{qk.shape}")
    qk = rearrange(qk, "b h g i j -> b (h g) i j")
    slash = sum_all_diagonal_matrix(qk) / qk.shape[-2]
    vertical = qk.mean(-2)
    # Logger.log(f"slash shape:{slash.shape}")
    # Logger.log(f"vertical shape:{slash.shape}")
...
def flex_prefill_attention_test_time(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    gamma: float,
    ...
    # 不测试全部使用flexprefill，用于测试所有长度文本的可行性
    ...
    torch.cuda.synchronize()
    end_search = (time.time() - search_time) * 1000
    Logger.log(f"search time:{(end_search):4}ms")
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    compuatation_time = (time.time() - computer_time) * 1000
    Logger.log(f"computation time:{compuatation_time}ms") 
    return attn_out, end_search, compuatation_time
def flex_prefill_attention(
    ...
    # Logger.log(f"block_idx shape:{block_idx}")
    if return_computational_ratio or True:
        ...
        # 存在上三角掩码
        total_block_num = (1 + num_blocks) * num_blocks * batch_size * num_q_heads / 2
